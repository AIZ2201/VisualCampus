...
class AllTranHandler  {
    ...
    fun handleAction(data:AllTranSend) :AllTranBack?{
        ...
        val senddata=AllTranSend(data.operation,data.cardNumber)
        // 序列化
        val jsonAdapter = moshi.adapter(AllTranSend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")
        val responseJson=sendToserver(requestjson).trim()

        val jsonAdapter1=moshi.adapter(AllTranBack::class.java)
...
data class AllTranSend(
    val operation: String,
    val cardNumber: Int,
)

data class AllTranBack(
    val status: String,
    val transactions: List<BorrowedBook>
)
