...
data class CommentSend(
    ...
    val password: String,
    val bookIsbn:String,
    val bookComment: BookCommand,
)

data class CommentBack(
    val status: String
)
...
class CommentHandler  {
    ...
    fun handleAction(data:CommentSend) :String?{

        val moshi = Moshi.Builder()
            .add(KotlinJsonAdapterFactory()) // 添加 KotlinJsonAdapterFactory
            .build()
        //进行加密
        val senddata=CommentSend(data.operation,data.cardNumber,encryptPassword(data.password,PUBLIC_KEY_STRING),data.bookIsbn,data.bookComment)
        // 序列化
        val jsonAdapter = moshi.adapter(CommentSend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")
        val responseJson=sendToserver(requestjson)

        val jsonAdapter1=moshi.adapter(CommentBack::class.java)
        val changestudentback=jsonAdapter1.fromJson(responseJson)
        return changestudentback?.status
    }
}
