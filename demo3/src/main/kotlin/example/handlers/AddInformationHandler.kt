...
class AddInformationHandler  {
    ...
    fun handleAction(data:AddStudentSend) :AddStudentBack?{

        val moshi = Moshi.Builder()
            .add(KotlinJsonAdapterFactory()) // 添加 KotlinJsonAdapterFactory
            .build()
        //进行加密
        val senddata=AddStudentSend(data.operation,data.cardNumber,encryptPassword(data.password,PUBLIC_KEY_STRING),data.student)
        // 序列化
        val jsonAdapter = moshi.adapter(AddStudentSend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")
        val responseJson=sendToserver(requestjson)

        val jsonAdapter1=moshi.adapter(AddStudentBack::class.java)
        val addstudentback=jsonAdapter1.fromJson(responseJson)
        return addstudentback
    }
}
