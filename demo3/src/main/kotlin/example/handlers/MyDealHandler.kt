...
data class MyDealSend(
    val operation:String,
    val cardNumber: Int,
    val password: String
)
data class MyDealBack(
    val status: String,
...
class MyDealHandler  {
    ...
    fun handleAction(data:MyDealSend) :MyDealBack?{

        val moshi = Moshi.Builder()
            .add(KotlinJsonAdapterFactory()) // 添加 KotlinJsonAdapterFactory
            .build()
        //进行加密
        val senddata=MyDealSend(data.operation,data.cardNumber,encryptPassword(data.password,PUBLIC_KEY_STRING))
        // 序列化
        val jsonAdapter = moshi.adapter(MyDealSend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")
        val responseJson=sendToserver(requestjson)
        println(responseJson)
        val jsonAdapter1=moshi.adapter(MyDealBack::class.java)
