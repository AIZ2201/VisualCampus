...
data class ChangeStudentSend(
    val operation: String,
    val cardNumber: Int,
    val password: String,
    val student:StudentBack
)

class changeInformationHandler  {
    ...
    fun handleAction(data:ChangeStudentSend) :ChangeStudentBack?{

        val moshi = Moshi.Builder()
            .add(KotlinJsonAdapterFactory()) // 添加 KotlinJsonAdapterFactory
            .build()
        //进行加密
        val senddata=ChangeStudentSend(data.operation,data.cardNumber,encryptPassword(data.password,PUBLIC_KEY_STRING),data.student)
        // 序列化
        val jsonAdapter = moshi.adapter(ChangeStudentSend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")
        val responseJson=sendToserver(requestjson)

        val jsonAdapter1=moshi.adapter(ChangeStudentBack::class.java)
        val changestudentback=jsonAdapter1.fromJson(responseJson)
        return changestudentback
    }
}
