...
class BorrowBookHandler  {
    ...
    fun handleAction(data: BorrowBookSend): BorrowedBook? {
        val moshi = Moshi.Builder()
            .add(KotlinJsonAdapterFactory())
            .build()

        val senddata = BorrowBookSend(data.operation, data.cardNumber, data.bookId)
        val jsonAdapter = moshi.adapter(BorrowBookSend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")

        val responseJson = sendToserver(requestjson)
        println("我接受到的：$responseJson")

        val jsonAdapter1 = moshi.adapter(BorrowBookBack::class.java)

        return try {
            jsonAdapter1.fromJson(responseJson)?.book
        } catch (e: Exception) {
            e.printStackTrace()
            null
        }
...
data class BorrowBookSend(
    val operation: String,
    val cardNumber: Int,
    val bookId:String
)

data class BorrowBookBack(
    val status:String,
    val book:BorrowedBook//类型与展示个人信息中的书籍类型相同，因为要放在个人信息里展示
)
