...
data class SearchGoodsBack(
    val status: String,
    val product: List<Goods>
)
data class Goods(
    val productID: Int,
    val name: String,
...
class ShowGoodsHandler  {
    ...
    fun handleAction(data:SearchGoodsSend) :SearchGoodsBack?{

        val moshi = Moshi.Builder()
            .add(KotlinJsonAdapterFactory()) // 添加 KotlinJsonAdapterFactory
            .build()
        //进行加密
        val senddata=SearchGoodsSend(data.operation,data.cardNumber,encryptPassword(data.password,PUBLIC_KEY_STRING),data.searchText,data.select)
        // 序列化
        val jsonAdapter = moshi.adapter(SearchGoodsSend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")
        val responseJson=sendToserver(requestjson)
    println(responseJson)
        val jsonAdapter1=moshi.adapter(SearchGoodsBack::class.java)
        val showgoodsback=jsonAdapter1.fromJson(responseJson)
        return  showgoodsback
    }
}
