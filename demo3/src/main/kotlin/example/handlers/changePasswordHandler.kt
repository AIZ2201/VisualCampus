...
class changePasswordHandler  {
    ...
    fun handleAction(data:ChangePasswordSend) :ChangePasswordBack?{

        val moshi = Moshi.Builder()
            .add(KotlinJsonAdapterFactory()) // 添加 KotlinJsonAdapterFactory
            .build()
        //进行加密
        val senddata=ChangePasswordSend(data.operation,data.cardNumber,encryptPassword(data.password,PUBLIC_KEY_STRING),data.oldPassword,data.newPassword)
        // 序列化
        val jsonAdapter = moshi.adapter(ChangePasswordSend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")
        val responseJson=sendToserver(requestjson)

        val jsonAdapter1=moshi.adapter(ChangePasswordBack::class.java)
