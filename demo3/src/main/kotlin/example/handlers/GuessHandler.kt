...
data class GuessBack(
    val status: String,
    val recommendedProducts: List<Goods>
)
...
class GuessHandler  {
    ...
    fun handleAction(data:GuessSend) :GuessBack?{

        val moshi = Moshi.Builder()
            .add(KotlinJsonAdapterFactory()) // 添加 KotlinJsonAdapterFactory
            .build()
        //进行加密
        val senddata=GuessSend(data.operation,data.cardNumber,encryptPassword(data.password,PUBLIC_KEY_STRING),data.searchText,data.select)
        // 序列化
        val jsonAdapter = moshi.adapter(GuessSend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")
        val responseJson=sendToserver(requestjson)
        println(responseJson)
        val jsonAdapter1=moshi.adapter(GuessBack::class.java)
