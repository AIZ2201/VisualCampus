...
class MybooksHandler  {
    ...
    fun handleAction(data:MybooksSend) :MybooksBack?{

        val moshi = Moshi.Builder()
            .add(KotlinJsonAdapterFactory()) // 添加 KotlinJsonAdapterFactory
            .build()
        //进行加密
       // val senddata=MybooksSend(data.operation,data.cardNumber,encryptPassword(data.password,PUBLIC_KEY_STRING))
        val senddata=MybooksSend(data.operation,data.cardNumber)
        // 序列化
        val jsonAdapter = moshi.adapter(MybooksSend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")
        val responseJson=sendToserver(requestjson).trim()

        val jsonAdapter1=moshi.adapter(MybooksBack::class.java)
        println(responseJson)

        val student=jsonAdapter1.fromJson(responseJson)
        return  student
    }
}
...
data class MybooksSend(
    val operation: String,
    val cardNumber: Int,
)

data class MybooksBack(
    val status: String,
...
data class LibraryUser(
    val userId: Int,
    val userName: String,
    val userMaxBorrowNum: Int,
    val userBorrowedNum: Int,
    val userBorrowed: List<BorrowedBook>
)

data class BorrowedBook(
    val tranId: String,
    val tranBorrowTime: String,
    val tranUserId: String,
    val tranBookId: String,
    val tranIsbn: String,
    val bookName:String,
    val tranDueTime:String
)
