...
class DeleteInformationHandler  {
    ...
    fun handleAction(data:DeleteStudentSend) :DeleteStudentBack?{

        val moshi = Moshi.Builder()
            .add(KotlinJsonAdapterFactory()) // 添加 KotlinJsonAdapterFactory
            .build()
        //进行加密
        val senddata=DeleteStudentSend(data.operation,data.cardNumber,encryptPassword(data.password,PUBLIC_KEY_STRING),data.studentCardNum)
        // 序列化
        val jsonAdapter = moshi.adapter(DeleteStudentSend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")
        val responseJson=sendToserver(requestjson)

        val jsonAdapter1=moshi.adapter(DeleteStudentBack::class.java)
        val delstudentback=jsonAdapter1.fromJson(responseJson)
        return delstudentback
    }
}
