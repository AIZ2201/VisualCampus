...
class ReturnBookHandler  {
    ...
    fun handleAction(data:ReturnBookSend) :String{

        val moshi = Moshi.Builder()
            .add(KotlinJsonAdapterFactory()) // 添加 KotlinJsonAdapterFactory
            .build()
        //进行加密
        val senddata=ReturnBookSend(data.operation,data.cardNumber,data.bookId)
        // 序列化
        val jsonAdapter = moshi.adapter(ReturnBookSend::class.java)
        ...
        val jsonAdapter1=moshi.adapter(ReturnBookBack::class.java)
...
data class ReturnBookSend(
    val operation: String,
    val cardNumber: Int,
    val bookId:String
)

data class ReturnBookBack(
    val status:String
)