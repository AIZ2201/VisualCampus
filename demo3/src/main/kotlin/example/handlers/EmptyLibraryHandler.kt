...
data class EmptyLibraryBack(
    val newsEvents: List<NewsEvent>,
    val actionEvents: List<ActionEvent>,
    val status: String
)

data class EmptyLibrarySend(
    val operation:String,
    val CardId:Int
)
class EmptyLibraryHandler  {
    ...
    fun handleAction(data:EmptyLibrarySend) :EmptyLibraryBack?{

        val moshi = Moshi.Builder()
            .add(KotlinJsonAdapterFactory()) // 添加 KotlinJsonAdapterFactory
            .build()
        //进行加密
        val senddata=EmptyLibrarySend(data.operation,data.CardId)
        // 序列化
        val jsonAdapter = moshi.adapter(EmptyLibrarySend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")
        val responseJson=sendToserver(requestjson).trim()

        val jsonAdapter1=moshi.adapter(EmptyLibraryBack::class.java)
        println(responseJson)
        val emptylibraryback=jsonAdapter1.fromJson(responseJson)
        return  emptylibraryback
    }
}
