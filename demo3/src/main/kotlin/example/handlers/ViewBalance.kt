...
data class ViewBalenceSend(
    val operation: String,
    val cardNumber: Int,
    val password: String
)

class ViewBalenceHandler  {
    ...
    fun handleAction(data:ViewBalenceSend) :ViewBalenceBack?{

        val moshi = Moshi.Builder()
            .add(KotlinJsonAdapterFactory()) // 添加 KotlinJsonAdapterFactory
            .build()
        //进行加密
        val senddata=ViewBalenceSend(data.operation,data.cardNumber,encryptPassword(data.password,PUBLIC_KEY_STRING))
        // 序列化
        val jsonAdapter = moshi.adapter(ViewBalenceSend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")
        val responseJson=sendToserver(requestjson)
        println(responseJson)
        val jsonAdapter1=moshi.adapter(ViewBalenceBack::class.java)
