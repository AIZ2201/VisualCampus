...
class DeleteBookHandler  {
    ...
    fun handleAction(data:DeleteBookSend) :String?{

        val moshi = Moshi.Builder()
            .add(KotlinJsonAdapterFactory()) // 添加 KotlinJsonAdapterFactory
            .build()
        val senddata=DeleteBookSend(data.operation,data.cardNumber,data.bookId)
        // 序列化
        val jsonAdapter = moshi.adapter(DeleteBookSend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")
        val responseJson=sendToserver(requestjson).trim()

        val jsonAdapter1=moshi.adapter(DeleteBookBack::class.java)
        println(responseJson)
        val returnbookback=jsonAdapter1.fromJson(responseJson)
        return  returnbookback?.status
    }
}
...
data class DeleteBookSend(
    val operation:String,
    val cardNumber:Int,
    val bookId:String
)

data class DeleteBookBack(
    val status:String
)