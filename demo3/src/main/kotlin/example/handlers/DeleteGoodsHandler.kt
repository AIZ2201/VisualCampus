...
class DeleteGoodsHandler  {
    ...
    fun handleAction(data: DeleteGoodsSend) :DeleteGoodsBack?{

        val moshi = Moshi.Builder()
            .add(KotlinJsonAdapterFactory()) // 添加 KotlinJsonAdapterFactory
            .build()
        //进行加密
        val senddata= DeleteGoodsSend(data.operation,data.cardNumber,encryptPassword(data.password,PUBLIC_KEY_STRING),data.goods)
        // 序列化
        val jsonAdapter = moshi.adapter(DeleteGoodsSend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")
        val responseJson=sendToserver(requestjson)
        println(responseJson)
        val jsonAdapter1=moshi.adapter(DeleteGoodsBack::class.java)
        val deletebooksback=jsonAdapter1.fromJson(responseJson)
        return  deletebooksback
    }
}
