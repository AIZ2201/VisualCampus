...
class showInformationHandler  {
   ...
   val PUBLIC_KEY_STRING: String = ""
    ...
    fun handleAction(data:StudentSend) :StudentBack?{

        val moshi = Moshi.Builder()
            .add(KotlinJsonAdapterFactory()) // 添加 KotlinJsonAdapterFactory
            .build()
        //进行加密
        val senddata:StudentSend=StudentSend(data.operation,data.cardNumber,encryptPassword(data.password,PUBLIC_KEY_STRING))
        // 序列化
        val jsonAdapter = moshi.adapter(StudentSend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")
        val responseJson=sendToserver(requestjson)

        val jsonAdapter1=moshi.adapter(StudentBack::class.java)
...
enum class PoliticialStat {
    Masses,//群众
    Members,//团员
    PartyMember,//党员
    UNKNOWN
}
data class StudentBack(
    val name: String,
    val cardNumber: Int,
    ...
    val major:String,
    val school:String,
    val studentStat:Status,
    val enrollment: String,
    val birthPlace:String,
    val politicalStat:PoliticialStat
)