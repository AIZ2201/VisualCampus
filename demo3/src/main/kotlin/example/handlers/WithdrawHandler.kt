...
class WithdrawHandler  {
    ...
    fun handleAction(data:WithdrawSend) :WithdrawBack?{

        val moshi = Moshi.Builder()
            .add(KotlinJsonAdapterFactory()) // 添加 KotlinJsonAdapterFactory
            .build()
        //进行加密
        val senddata=WithdrawSend(data.operation,data.cardNumber,encryptPassword(data.password,PUBLIC_KEY_STRING),data.amount)
        // 序列化
        val jsonAdapter = moshi.adapter(WithdrawSend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")
        val responseJson=sendToserver(requestjson)
        println(responseJson)
        val jsonAdapter1=moshi.adapter(WithdrawBack::class.java)
