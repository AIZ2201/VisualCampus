...
import example.handlers.Book

data class ChangeBookSend(
    val operation:String,
    val CardId:Int,
    val book:Book
)

data class ChangeBookBack(
    val status:String,
)

class ChangeBookHandler  {
    ...
    fun handleAction(data:ChangeBookSend) :String?{
        ...
        //进行加密
        val senddata=ChangeBookSend(data.operation,data.CardId,data.book)
        // 序列化
        val jsonAdapter = moshi.adapter(ChangeBookSend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")

        //反序列化
        val responseJson=sendToserver(requestjson)
        val jsonAdapter1 = moshi.adapter(ChangeBookBack::class.java)
