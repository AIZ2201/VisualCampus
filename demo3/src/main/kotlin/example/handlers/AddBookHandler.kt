...
data class AddBookSend(
    val operation:String,
    val CardId:Int,
    val newBook:NewBook
)

data class AddBookBack(
    val status:String,
)
...
class AddBookHandler  {
    ...
    fun handleAction(data:AddBookSend) :String?{
        ...
        //进行加密
        val senddata=AddBookSend(data.operation,data.CardId,data.newBook)
        // 序列化
        val jsonAdapter = moshi.adapter(AddBookSend::class.java)
        val requestjson = jsonAdapter.toJson(senddata)
        println("toJson = $requestjson")

        //反序列化
        val responseJson=sendToserver(requestjson)
        val jsonAdapter1 = moshi.adapter(AddBookBack::class.java)
