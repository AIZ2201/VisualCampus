...
@Composable
fun StudentSideNavBar(selectedMenu: String, onMenuClick: (String) -> Unit) {
    val menuItems = listOf("展示个人学籍信息", "修改密码") // 添加“修改密码”菜单项
...
@Composable
fun Student_MainContent(user: User, selectedMenu: String) {
    println("Current selectedMenu: $selectedMenu") // 打印当前选中的菜单项
    when (selectedMenu) {
        "展示个人学籍信息" -> StudentDisplayAcademicInfoContent(user) // 展示个人学籍信息
        "修改密码" -> ChangePassword(user)
...
@Composable
fun StudentDisplayAcademicInfoContent(user: User) {
...
@Composable
@Preview
fun StudentInformation_Select(user: User) {
    val (selectedMenu, setSelectedMenu) = remember { mutableStateOf("展示个人学籍信息") }

    MaterialTheme {
        Row(modifier = Modifier.fillMaxHeight()) {
            StudentSideNavBar(selectedMenu, onMenuClick = { menu ->
                setSelectedMenu(menu)
            })
            Student_MainContent(user, selectedMenu = selectedMenu)
        }
    }
}
